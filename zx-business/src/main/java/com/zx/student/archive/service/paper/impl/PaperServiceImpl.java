...
import com.zx.student.archive.domain.dto.paper.PaperVolumeDTO;
import com.zx.student.archive.domain.dto.paper.PaperSectionDTO;
import com.zx.student.archive.domain.dto.paper.PaperQuestionGroupDTO;
...
import com.zx.student.archive.domain.bo.question.QuestionIdBO;
import com.zx.student.archive.domain.dto.question.QuestionInfoDTO;
import com.zx.student.archive.utils.PaperCodeUtils;
...
public class PaperServiceImpl implements IPaperService {
    ...
    public void executeGeneratePackageAsync(Integer paperId) {
        ...
        com.zx.student.archive.domain.dto.paper.PackageTaskInfo taskInfo = packageTaskService.getTask(paperId);
        if (taskInfo != null) {
        ...
        try {
            ...
            try {
                ...
                com.zx.common.utils.oss.IOssService.UploadResult quickStartUploadResult = ossUtil
            ...
            try {
                ...
                java.util.List<com.zx.common.utils.oss.IOssService.PartInfo> existingUploadedParts = null;
                ...
                if (taskInfo != null && taskInfo.getUploadId() != null && !taskInfo.getUploadId().isEmpty()
                    ...
                    try {
                        // 查询OSS上已上传的分片
                        java.util.List<com.zx.common.utils.oss.IOssService.PartInfo> ossParts = ossUtil
                ...
                // 使用支持断点续传的上传方法
                com.zx.common.utils.oss.IOssService.UploadResult uploadResult;
                try {
                    ...
                    if (taskInfo != null) {
                        ...
                        if (uploadResult.getUploadedParts() != null && !uploadResult.getUploadedParts().isEmpty()) {
                            ...
                            for (com.zx.common.utils.oss.IOssService.PartInfo part : uploadResult
                                    .getUploadedParts()) {
                    ...
                    if (taskInfo != null && existingUploadId != null) {
                        ...
                        if (existingUploadedParts != null && !existingUploadedParts.isEmpty()) {
                            java.util.List<java.util.Map<String, Object>> partsJson = new java.util.ArrayList<>();
                            for (com.zx.common.utils.oss.IOssService.PartInfo part : existingUploadedParts) {
                                java.util.Map<String, Object> partMap = new java.util.HashMap<>();
                                partMap.put("partNumber", part.getPartNumber());
                                partMap.put("eTag", part.getETag());
                                partMap.put("size", part.getSize());
                ...
                com.zx.student.archive.domain.dto.paper.PackageTaskInfo errorTaskInfo = packageTaskService
                        .getTask(paperId);
            ...
            // 清除上传状态（任务完成，不再需要续传）
            com.zx.student.archive.domain.dto.paper.PackageTaskInfo finalTaskInfo = packageTaskService
                    .getTask(paperId);
    ...
    public Integer createPaperWithFullData(PaperFullDataBO fullDataBO) throws ServiceException {
        ...
        paper.setIntroAudioUrl(com.zx.common.utils.oss.exam.question.OssUtil.cleanOssUrl(paper.getIntroAudioUrl()));
        paper.setTrialListenAudioUrl(
                com.zx.common.utils.oss.exam.question.OssUtil.cleanOssUrl(paper.getTrialListenAudioUrl()));
        paper.setTrialIntroAudioUrl(
                com.zx.common.utils.oss.exam.question.OssUtil.cleanOssUrl(paper.getTrialIntroAudioUrl()));
        paper.setOperateListenImageUrl(
                com.zx.common.utils.oss.exam.question.OssUtil.cleanOssUrl(paper.getOperateListenImageUrl()));
        ...
        if (fullDataBO.getVolumes() != null && !fullDataBO.getVolumes().isEmpty()) {
            ...
            for (VolumeDataBO volumeBO : fullDataBO.getVolumes()) {
                ...
                volume.setVolumeAudioUrl(
                        com.zx.common.utils.oss.exam.question.OssUtil.cleanOssUrl(volume.getVolumeAudioUrl()));
                ...
                if (volumeBO.getSections() != null && !volumeBO.getSections().isEmpty()) {
                    ...
                    for (SectionDataBO sectionBO : volumeBO.getSections()) {
                        ...
                        // 清洗大题音频 URL
                        section.setInstructionAudioUrl(com.zx.common.utils.oss.exam.question.OssUtil
                        ...
                        if (sectionBO.getQuestionGroups() != null && !sectionBO.getQuestionGroups().isEmpty()) {
                            ...
                            for (PaperQuestionGroupBO groupBO : sectionBO.getQuestionGroups()) {
                                ...
                                // 清洗题目组音频 URL
                                group.setAudioUrl(com.zx.common.utils.oss.exam.question.OssUtil
        ...
        if (fullDataBO.getIntermissions() != null && !fullDataBO.getIntermissions().isEmpty()) {
            ...
            for (IntermissionDataBO intermissionBO : fullDataBO.getIntermissions()) {
                ...
                // 清洗中场音频 URL
                intermission.setIntermissionAudioUrl(com.zx.common.utils.oss.exam.question.OssUtil
